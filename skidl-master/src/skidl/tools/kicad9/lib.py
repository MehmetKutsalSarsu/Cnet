...
def default_lib_paths():
    ...
    PATCHED for PCB-Generator assets-only mode:
    - No '.' (CWD) pollution.
    - KICAD{N}_SYMBOL_DIR env var is honoured if set, but missing it is silent.
    - The generated SKiDL script clears this list via
      lib_search_paths[KICAD].clear() and appends only assets/librarys/ paths.
    See plan_assets_only_libraries.md for rationale.
    """
    kicad_version = __name__.split(".")[-2][len("kicad"):]

    paths = []
    env_path = os.environ.get(f"KICAD{kicad_version}_SYMBOL_DIR")
    if env_path:
        paths.append(env_path)
    # Intentionally no warning when env var is missing.
    return paths
