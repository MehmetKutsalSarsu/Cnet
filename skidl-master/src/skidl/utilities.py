...
def expand_path(path):
    """Expand a filesystem path, resolving env vars, ``~``, and relative segments.

    Returns an empty string when *path* is ``None`` or blank, preventing
    downstream ``os.walk`` / ``os.path.expanduser`` calls from receiving
    invalid inputs.
    """
    if path is None:
        return ""
    ...
    if not path_str:
        return ""
    return abspath(normpath(expandvars(expanduser(path_str))))
...
def find_and_open_file(
    ...
    for path in paths:
        if is_url(path):
            ...
            # Resolve and validate the search path before walking.
            valid_path = expand_path(path)
            if not valid_path:
                # Path derived from an unset or empty env variable — skip it.
                # A downstream SKiDL error will be raised if the symbol is
                # not found in any valid search path.
                continue
            ...
            for root, dirnames, filenames in os.walk(valid_path):
