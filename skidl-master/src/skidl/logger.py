...
class SkidlLogger(logging.getLoggerClass()):
    ...
    def report_summary(self, phase_desc):
        ...
        Only emits output when there are actual warnings or errors.
        Silent on success to prevent downstream parsers from triggering
        false positives on messages like ``"0 errors found"``.

        Args:
            phase_desc (str): Description of the phase being summarized (e.g., "generating netlist").
        """
        total_warnings = self.warning.count + self.bare_warning.count
        total_errors = self.error.count + self.bare_error.count

        if total_errors == 0 and total_warnings == 0:
            # Intentionally silent — no output on clean runs.
            return

        if total_warnings > 0:
            self.summary(
                f"{total_warnings} warning(s) found while {phase_desc}."
            )
        if total_errors > 0:
            self.summary(
                f"{total_errors} error(s) found while {phase_desc}.\n"
            )
...
# Clamp logger level via environment variable.  Default is ERROR to prevent
# stray INFO/WARNING messages from triggering false positives in downstream
# parsers.  Set SKIDL_LOG_LEVEL=DEBUG to restore verbose output.
_skidl_log_level = getattr(
    logging,
    os.environ.get("SKIDL_LOG_LEVEL", "ERROR").upper(),
    logging.ERROR,
)
for _lgr in (rt_logger, erc_logger):
    _lgr.setLevel(_skidl_log_level)
    for _h in _lgr.handlers:
        _h.setLevel(_skidl_log_level)
